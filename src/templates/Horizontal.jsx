    ...
    <>
      <div className='w-full h-[60vh] p-5 '>
        <div className='mb-5 flex justify-between'>
          <h1 className='text-zinc-200 font-semibold text-lg'>{heading}</h1>

          {/* <Dropdown title="Filter" options = {["TV" , "Movie" , "All"]} /> */}
        </div>
        <div className='w-full h-full flex gap-5 shrink-0 overflow-x-auto '>
          {data.length > 0 ? data.map((e, i) => <Link to={`/${e.media_type}/details/${e.id}`}  key={i} className={`min-w-[15%] bg-gradient-to-b from-black to-zinc-900 rounded-lg overflow-hidden h-[45vh] `}>

            <h1 className=' h-full font-semibold w-full text-base flex flex-col items-start text-white '>
              ...
              <div className='px-3 py-1 overflow-auto'>
                ...
                <p className='text-sm font-normal tracking-tighter mt-3'>{e.overview.slice(0, 60)}... <Link className='text-gray-700'>more</Link></p>
              </div>
